...
describe("Count", function() {
    ...
    before(co.wrap(function * () {
        var c = new CountModel({
            value: gCount
        });
        yield[c.save(), authHelper.createUser(), ];
    }));

    describe("Anonymous calls", function() {
        it("should return 401 /value", function(done) {
            request.get("/value").accept("json").expect(401).end(done);
        });

        it("should return 401 /inc", function(done) {
            request.get("/inc").accept("json").expect(401).end(done);
        });

        it("should return 401 /dec", function(done) {
            request.get("/dec").accept("json").expect(401).end(done);
        });
    });
    ...
    describe("Auth calls", function() {
        ...
        it("should fetch value", function(done) {
            request.get("/value").accept("json").expect(200).end(function(err, res) {
                if (err) {
                    return done(err);
                }
        ...
        it("should increment value", function(done) {
            request.get("/inc").accept("json").expect(200).end(function(err, res) {
                if (err) {
                    return done(err);
                }
        ...
        it("should decrement value", function(done) {
            request.get("/dec").accept("json").expect(200).end(function(err, res) {
                if (err) {
                    return done(err);
                }
